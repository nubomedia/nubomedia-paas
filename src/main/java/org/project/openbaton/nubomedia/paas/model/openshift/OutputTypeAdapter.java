...
public class OutputTypeAdapter extends TypeAdapter<Output> {
  ...
  public Output read(JsonReader in) throws IOException {
    ...
    while (in.hasNext()) {
      ...
      if (in.nextName().equals("pushSecret")) secId = this.readSecID(in);
    }
    in.endObject();
  ...
  private SecretID readSecID(JsonReader in) throws IOException {
    ...
    in.beginObject();
    while (in.hasNext()) {
      if (in.nextName().equals("name")) res.setName(in.nextString());
    }
    in.endObject();
  ...
  private BuildElements readBuildElement(JsonReader in) throws IOException {
    ...
    while (in.hasNext()) {
      if (in.nextName().equals("kind")) be.setKind(in.nextString());
      if (in.nextName().equals("name")) be.setName(in.nextString());
    }
